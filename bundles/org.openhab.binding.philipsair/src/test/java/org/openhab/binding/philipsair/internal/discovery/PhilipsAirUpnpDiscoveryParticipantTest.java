...
public class PhilipsAirUpnpDiscoveryParticipantTest extends JavaTest {
    ...
    public void testGetThingUID() throws URISyntaxException {
        ...
        assertNotNull(thing);
        assertThat(thing.getAsString(), containsString(PhilipsAirBindingConstants.THING_TYPE_AC2889_10.getAsString()));

        when(modelDetails.getModelNumber()).thenReturn("AC3829");
        thing = participant.getThingUID(device);
        assertNotNull(thing);
        assertThat(thing.getAsString(), containsString(PhilipsAirBindingConstants.THING_TYPE_AC3829_10.getAsString()));

        when(modelDetails.getModelNumber()).thenReturn("AC1214");
        thing = participant.getThingUID(device);
        assertNotNull(thing);
        assertThat(thing.getAsString(), containsString(PhilipsAirBindingConstants.THING_TYPE_AC1214_10.getAsString()));

        when(modelDetails.getModelNumber()).thenReturn("AC2729");
        thing = participant.getThingUID(device);
        assertNotNull(thing);
        assertThat(thing.getAsString(), containsString(PhilipsAirBindingConstants.THING_TYPE_AC2729.getAsString()));

        when(modelDetails.getModelNumber()).thenReturn("AC3333");
        thing = participant.getThingUID(device);
        assertNotNull(thing);
        assertThat(thing.getAsString(), containsString(PhilipsAirBindingConstants.THING_TYPE_UNIVERSAL.getAsString()));
    }
}
